...
{
    // ============================ Configuration ============================
    private const int DefaultMemMb = 1024;          // дефолтный размер ранна (если --runsize не задан)
    private const int DefaultFanIn = 128;           // сколько раннов сливаем за один проход
    private const int IoBufferSizeBytes = 1 << 20;       // 1 MiB
    private const int ProgressIntervalMs = 500;           // период обновления статуса
    ...
    private sealed class Options
    {
        ...
        public required int RunSizeMb { get; init; }
        ...
        public void PrintHeader()
        {
            ...
            Console.WriteLine($"RunSize : {RunSizeMb} MB");
            Console.WriteLine($"Fan-in  : {FanIn}\n");
        }
    }
    ...
    private static int Main(string[] args)
    {
        ...
        if (!Options.TryParse(args, out var opt, out var err))
        {
    ...
    private static List<string> CreateRuns(Options opt, CancellationToken token)
    {
        ...
        long targetBytes = (long)opt.RunSizeMb << 20; // MB -> bytes
        ...
        // Writers: сортируют батчи и сбрасывают во временные файл-ранны
        ...
        // Reader: режем ранны по ФАКТИЧЕСКИ прочитанным байтам (fs.Position)
        ...
        while ((line = sr.ReadLine()) is not null)
        {
            ...
            long consumed = fsIn.Position - runStartPos;
    ...
    private sealed class RunReader : IDisposable
    {
        ...
        public bool Advance()
        {
            while (true)
            {
                ...
                // битую строку просто пропускаем; при желании можно добавить лог/политику
            }
        }
